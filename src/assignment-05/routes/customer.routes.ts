...
router.post("/create",async(req,res)=>{
    try{
        ...
        const employee = await customerService.createCustomer(first_name,last_name,email,password);
        res.status(201).json({
...
router.put("/:id",async(req,res)=>{
    try{
        ...
        const result = await customerService.updateCustomer(id,first_name,last_name,email,password);
        if(result === 0){
